...
const PotonganEditPage = () => {
    ...
    const handlePotonganServiceEdit = () => {
        ...
        setLoading(true);
        PotonganService.edit(ID_Potongan, dataToSend)
            .then((response) => {
                success(`Berhasil mengubah data potongan ${ID_Potongan}`);
                setTimeout(() => navigate("/potongan"), 1000);
            })
            .catch((err) => {
                const errorMsg = err.response?.data?.message || "Gagal mengupdate potongan.";
                error(errorMsg);
    ...
    const handlePotonganServiceDelete = () => {
        ...
        if (isDelete) {
            setLoading(true);
            PotonganService.delete(ID_Potongan, potongan)
                .then(() => {
                    success(`Berhasil menghapus potongan ${ID_Potongan}`);
                    setTimeout(() => navigate("/potongan"), 1000);
                })
                .catch((err) => {
                    const errorMsg = err.response?.data?.message || "Gagal menghapus potongan.";
                    error(errorMsg);
                })
                .finally(() => {
                    setLoading(false);
